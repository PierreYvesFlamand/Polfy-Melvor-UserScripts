...
function BetterCompletionLog() {
	...
	tippy: tippy,
        ...
        baseMediaUrl: "https://cdn.melvor.net/core/v018/",
        questionSVG: "assets/media/main/question.svg",

        // Items
        getItemsLogContainer: () => document.querySelectorAll("#itemlog-container .bank-item"),
        items: items,
        resetItemLog() {
            itemLogLoaded = false;
            buildItemLog();
        ...
        // Monsters
        getMonstersLogContainer: () => [
            ...document.querySelectorAll("#monsterlog-container .monster-item"),
            ...document.querySelectorAll("#monsterlog-container .monster-item-boss"),
        ],
        MONSTERS: MONSTERS,
        resetMonsterLog() {
        ...
        // Pets
        getPetsLogContainer: () => document.querySelectorAll("#petlog-container .monster-item"),
        PETS: PETS,
        resetPetLog() {
            petLogLoaded = false;
            buildPetLog();
    ...
    const tmpFunction = showCompletionCategory;
    showCompletionCategory = (category) => {
        tmpFunction(category);

        switch (category) {
            case 2:
                MELVOR.resetItemLog();
                setTimeout(() => {
                    updateLog(MELVOR.getItemsLogContainer(), MELVOR.items, TRANSLATIONS.ITEMS);
                }, 1200);
                break;

            case 3:
                MELVOR.resetMonsterLog();
                updateLog(MELVOR.getMonstersLogContainer(), MELVOR.MONSTERS, TRANSLATIONS.MONSTERS);
                break;

            case 4:
                MELVOR.resetPetLog();
                updateLog(MELVOR.getPetsLogContainer(), MELVOR.PETS, TRANSLATIONS.PETS, true);
                break;
        }
    };

    function updateLog(containers, datas, trans, isPet = false) {
        containers.forEach((node) => {
            const img = node.querySelector("img");
            const newImg = img.cloneNode();

            const nodeId = node.id.split("-")[node.id.split("-").length - 1];

            if (newImg.src === MELVOR.baseMediaUrl + MELVOR.questionSVG) {
                newImg.src = (!isPet ? MELVOR.baseMediaUrl : "") + datas[nodeId].media;
                newImg.style.opacity = "33%";
            }
    ...
    // Code to get EN names
    /*
    let itemNames = [];
    items.forEach((item) => {
        itemNames.push(item.name);
    });
    console.log(JSON.stringify(itemNames, " ", 2));

    let monsterNames = [];
    MONSTERS.forEach((monster) => {
        monsterNames.push(monster.name);
    });
    console.log(JSON.stringify(monsterNames, " ", 2));

    let petNames = [];
    PETS.forEach((pet) => {
        petNames.push(pet.name);
    });
    console.log(JSON.stringify(petNames, " ", 2));
	*/

    // Names translations for language support and link to wiki
    const TRANSLATIONS = {
        ITEMS: [
            "Normal Logs",
            "Oak Logs",
...
(() => {
    function loadScript() {
        ...
        if (typeof isLoaded !== typeof undefined && isLoaded) {
            ...
            scriptElem.textContent = `try {(${BetterCompletionLog})();} catch (e) {console.log(e);}`;
